...
object UserFavouritesFilms {
  ...
  def register(movies:DataFrame, ratings:DataFrame, movieRatings:DataFrame ) :  DataFrame = {
    ...
    val userFilms = NewDayContext.sqlContext.sql("select r.userId as userId, m.title as title, mr.avg as avg, CONCAT(mr.avg, ' ' ,m.title) as value " +
    ...
    val fullListDF = NewDayContext.sqlContext.sql("select userId, sort_array(collect_list(value), false) AS collected FROM user_films GROUP BY userId")
